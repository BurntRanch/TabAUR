...
void log_println(int log, const fmt::text_style& ts, std::string_view fmt, Args&&... args) {
    switch (log) {
        case LOG_ERROR: fmt::print(BOLD_TEXT(color.red), "ERROR: "); break;
        case LOG_WARN: fmt::print(BOLD_TEXT(color.yellow), "Warning: "); break;
        case LOG_INFO: fmt::print(BOLD_TEXT(color.cyan), "Info: "); break;
        case LOG_DEBUG:
            if (!config->debug)
                return;
